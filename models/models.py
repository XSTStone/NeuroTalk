...
class ResBlock(torch.nn.Module):  # 残差块
    ...
    def __init__(self, h):
        ...
        self.conv_pre = weight_norm(
            Conv1d(h.in_ch,
                   ...
                   3, 1,
                   padding=get_padding(3, 1)))  # 定义一个前置卷积层

        self.GRU = nn.GRU(h.ch_init_upsample // 2,
                          h.ch_init_upsample // 4,
                          num_layers=1,
                          batch_first=True,
                          bidirectional=True)  # 定义GRU

        self.ups = nn.ModuleList()  # 创建一个空的ModuleList,用于存储多个转置卷积层
...
class Discriminator(torch.nn.Module):
    ...
    def forward(self, x):
        # print('=================Model D forward doing=================')
        # print('222, x.shape: ', x.shape)
        x = self.conv_pre(x)
        # print('224, x.shape: ', x.shape)
        # print('self.num_downsamples: ', self.num_downsamples)

        for i in range(self.num_downsamples):
            x = F.leaky_relu(x, LRELU_SLOPE)
            # print('228, x.shape: ', x.shape)
            x = self.downs[i](x)
            # print('230, x.shape: ', x.shape)
